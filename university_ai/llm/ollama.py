...
class OllamaEngine:
    ...
    def generate(self, request: LlmRequest) -> LlmResponse:
        payload={"model":self._model,"prompt":request.prompt,"system":request.system_prompt or "","stream":False,"options":{"temperature":request.temperature,"num_predict":request.max_tokens}}
        started=time.monotonic(); result=self._json("/api/generate",payload)
        return LlmResponse(result.get("response", "").strip(),self._model,self.runtime,datetime.now(UTC),time.monotonic()-started,{"done_reason":result.get("done_reason")})
