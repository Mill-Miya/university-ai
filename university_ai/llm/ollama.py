...
class OllamaEngine:
    ...
    def generate(self, request: LlmRequest) -> LlmResponse:
        ...
        return LlmResponse(content,self._model,self.runtime,datetime.now(UTC),time.monotonic()-started,{"done_reason":result.get("done_reason"),"load_duration_ns":result.get("load_duration",0),"prompt_eval_duration_ns":result.get("prompt_eval_duration",0),"eval_duration_ns":eval_duration,"eval_count":eval_count,"tokens_per_second":eval_count/(eval_duration/1e9) if eval_duration else 0,"thinking_present":bool(message.get("thinking"))})
